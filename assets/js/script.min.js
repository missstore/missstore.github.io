let TodoConteudo = [],ProximaImagemIndex, PreviousImagemIndex, Modificada, selec, LocalizacoesAtuais = [],timerStamp;
...
req.send();
req.onload = function() {
	sessionStorage.setItem("grupos", atou(JSON.parse(req.response).content));
	wwm(".produtos .loadercontainer").then(function() {
		if (window.location.pathname == "/") {
			new RequestImagens("/croppeds/,/saias/,/vestidos/,/calcas/,/conjuntos/".shuffle(), true);
		} else {
			new RequestImagens(window.location.pathname + "/");
		}
	});
};
...
function MoverImagens(elmnt) {
	...
	function TouchStart(e) {
		e = e || window.event;
		pos2 = parseInt(e.targetTouches[0].clientX);
		pos3 = parseInt(e.targetTouches[0].clientX) - 200;
		menosesse = parseInt(e.targetTouches[0].clientX) - 200;
		eles = [];
		elmnt.style.backgroundPositionX.split(", ").forEach(function(e) {
			eles.push(e);
		});
		oindex = eles.findIndex(f => f == "0px");
		document.ontouchend = TouchEnds;
		document.ontouchmove = TouchMove;
	}

	function TouchMove(e) {
		e = e || window.event;
		pos1 = pos2 - parseInt(e.changedTouches[0].clientX);
		pos2 = parseInt(e.changedTouches[0].clientX);
		pos3 = parseInt(e.changedTouches[0].clientX) - 200;
		/*let respos = menosesse.toString().startsWith("-") ? Number(menosesse.toString().replace("-", "") - pos3) : menosesse - pos3;*/
		let resneg = menosesse - pos3;
		/* soma = pos1 >= 0 ? resneg - resneg * 2 : respos - respos * 2; */
		soma = resneg - resneg * 2;
		eles[oindex] = soma + "px";
		elmnt.style.backgroundPositionX = eles.toString();
	}

	function TouchEnds() {
		if (soma > 0) proximoindex = oindex - 1 == -1 ? eles.length - 1 : oindex - 1;
		else proximoindex = oindex + 1 == eles.length ? 0 : oindex + 1;
		if (soma >= 65 || soma <= -65) {
			eles[proximoindex] = "0px";
			eles[oindex] = "200px";
			elmnt.children[0].querySelectorAll("*").forEach(function(e, i) {
				i == proximoindex ? e.classList.add("nesse") : e.classList.remove("nesse");
			});
		} else {
			eles[oindex] = "0px";
		}
	...
	if (!document.querySelector(".carrinhoconteudo.aberto") && !document.querySelector(".categorias.aberto")) {
		...
		document.onscroll = function() {
			if (scrollY < currentPostionY) {
				...
				currentPostionY = scrollY;
				document.querySelector(".header").style.transform = "translate(0, -45px)";
			}
		};
	}
}

function abrirFull(foto, multiplos) {
	multiplos = multiplos || false;
	...
	function DoIt() {
		if (multiplos) {
			let imgstoload = foto.length;
			document.querySelector(".fotofullviewing").classList.remove("single");
			foto.forEach(function(e, i) {
				if (i == 0) {
					e.startsWith(" ") ? e = e.replace(" ", "") : null;
					document.querySelector(".fotofullviewing img").src = e;
					document.querySelector(".fotofullviewing img").onload = function() {
						if (imgstoload <= 1) {
							document.querySelector(".verfotofull").classList.remove("carregando");
						} else {
							imgstoload = Number(imgstoload - 1);
						}
					};
				} else {
					e.startsWith(" ") ? e = e.replace(" ", "") : null;
					...
					document.querySelector(".fotofullviewing .asimagensfull").append(novaimg);
			...
			document.querySelector(".fotofullviewing").classList.add("single");
			document.querySelector(".fotofullviewing img").src = foto;
...
function verificarsegrupoexiste(qual) {
	let bool = false;
	JSON.parse(sessionStorage.grupos).grupos.forEach(function(item) {
		if (item.grupo == qual) {
			bool = true;
		}
	});
...
function AtualizarImagemAtual(DivPai, direcao) {
	TodoConteudo = [];
	DivPai.style.background.split(",").forEach(function(e) {
		if (e.startsWith(" ")) {
	...
	function ModificarImagemAtual(sentido) {
		Modificada = '';
		if (sentido == "frente") {
			TodoConteudo.forEach(function(e, i) {
				if (e.includes(" 0px ")) {
					ProximaImagemIndex = i + 1 == TodoConteudo.length ? 0 : i + 1;
				}
			})
			sentido = ProximaImagemIndex;
		} else {
			TodoConteudo.forEach(function(e, i) {
				if (e.includes(" 0px ")) {
					PreviousImagemIndex = i == 0 ? TodoConteudo.length - 1 : i - 1;
				}
			})
			sentido = PreviousImagemIndex;
		}
		TodoConteudo.forEach(function(e, i) {
			if (i == sentido) {
				e.includes(" -200px ") ? e = e.replace(" -200px ", " 0px ") : e = e.replace(" 200px ", " 0px ");
			} else {
				direcao == "frente" ? e = e.replaceAll(" 0px ", " 200px ") : e = e.replaceAll(" 0px ", " -200px ");
			}
			i == TodoConteudo.length - 1 ? Modificada += e : Modificada == null ? Modificada = e + "," : Modificada += e + ",";
		})
		DivPai.style.background = Modificada;
		DivPai.children[0].querySelectorAll("*").forEach(function(e, i) {
			if (i == sentido) {
				e.classList.add("nesse");
			} else {
				e.classList.remove("nesse");
			}
		});
	}
	ModificarImagemAtual(direcao);
}
...
function RequestImagens(path, mutiplos) {
	...
	function criarImages(caminho) {
		let req = new XMLHttpRequest();
		req.open("GET", "https://missstore.github.io/assets/img" + caminho + "info.json", true);
		req.onload = function() {
			JSON.parse(req.response).info.forEach(function(e) {
				if (verificarsegrupoexiste(e.grupo)) {
					if (document.querySelector("div[grupo='" + e.grupo + "']")) {
						let inic = document.querySelector("div[grupo='" + e.grupo + "']").style.background;
						document.querySelector("div[grupo='" + e.grupo + "']").style.background = inic +
							"," +
							'url("/assets/img' + caminho + e.src + '") 200px center no-repeat';
						document.querySelector("div[grupo='" + e.grupo + "']").parentElement.querySelector(".dos-btn-f-b").removeAttribute("style")
						document.querySelector("div[grupo='" + e.grupo + "']").children[0].append(document.createElement("span"));
						document.querySelector("div[grupo='" + e.grupo + "']").children[0].removeAttribute("style");
					} else {
						...
						novaImg.setAttribute("grupo", e.grupo);
						novaImg.style.background = 'url("/assets/img' + caminho + e.src + '") 0px center no-repeat';
						novaImg.addEventListener("click", function() {
							let BgI = this.style.backgroundImage;
							let asimagens = [];
							BgI.split(",").length > 1 ? BgI.split(",").forEach(function(e) {
									e.startsWith(" ") ? e = e.replace(" ", "") : null;
									e = e.substring(5, e.length - 2);
									asimagens.push(e);
								}) :
								asimagens = BgI.substring(5, BgI.length - 2);
							new abrirFull(asimagens, BgI.split(",").length > 1 ? true : false);
						});
						let DivBolinhas = document.createElement("div");
						DivBolinhas.classList.add("bolinhas");
						DivBolinhas.style = "display: none;";
						let oSpanDab = document.createElement("span");
						oSpanDab.classList.add("nesse");
						DivBolinhas.append(oSpanDab);
						novaImg.append(DivBolinhas);
						divProdutoInteiro.append(novaImg);
						let DivPrevNext = document.createElement("div");
						DivPrevNext.classList.add("dos-btn-f-b");
						let n_u_a = navigator.userAgent.toLocaleLowerCase(),
							isMobile = navigator.userAgentData.mobile;
						n_u_a.match("android") || n_u_a.match("iphone") || n_u_a.match("ipad") || isMobile ? (DivPrevNext.classList.add("mobile"), MoverImagens(novaImg)) : null;
						DivPrevNext.style.display = "none";
						let ImgPrev = document.createElement("img");
						ImgPrev.classList.add("btn-previous");
						ImgPrev.src = "data:image/png;base64,";
						ImgPrev.addEventListener("click", function() {
							let DivPai = this.parentElement.parentElement.querySelector(".produtoimagen");
							AtualizarImagemAtual(DivPai, "atras");
						});
						DivPrevNext.append(ImgPrev);
						let ImgNext = document.createElement("img");
						ImgNext.classList.add("btn-forward");
						ImgNext.src = "data:image/png;base64,";
						ImgNext.addEventListener("click", function() {
							let DivPai = this.parentElement.parentElement.querySelector(".produtoimagen");
							AtualizarImagemAtual(DivPai, "frente");
						});
						DivPrevNext.append(ImgNext);
						divProdutoInteiro.append(DivPrevNext);
						...
						BtnAdicionar.addEventListener("click", function() {
							let BgI = this.parentElement.querySelector(".produtoimagen").style.backgroundImage;
							let s = [];
							BgI.split(",").length > 1 ? BgI.split(",").forEach(function(e, i) {
									e.startsWith(" ") ? e = e.replace(" ", "") : null;
									e = e.substring(5, e.length - 2);
									i == 0 ? s = e : null;
								}) :
								s = BgI.substring(5, BgI.length - 2);
						...
						BtnComprar.addEventListener("click", function() {
							...
							BgI.split(",").length > 1 ? BgI.split(",").forEach(function(e, i) {
								...
								imagem = BgI.substring(5, BgI.length - 2);
							let texto = 'Olá!%0aGostaria de fazer uma compra%0a';
							texto += '%0aFoto do produto: ' + "https://missstore.github.io".concat(imagem);;
					...
					if (e.nome) {
						...
						let novaImg = document.createElement("div");
						novaImg.classList.add("produtoimagen");
						novaImg.style.backgroundImage = 'url("/assets/img' + caminho + e.src + '")';
						novaImg.addEventListener("click", function() {
							let BgI = this.style.backgroundImage;
							let asimagens = [];
							BgI.split(",").length > 1 ? BgI.split(",").forEach(function(e) {
				...
				let img = new Image();
				img.src = '/assets/img' + caminho + e.src;
...
function VerMouseDownCategorias(elmselec) {
	document.onmousedown = function(e) {
		if (e.target.className) {
			if (e.target.className != "catCont" && e.target.className != "far fa-images" && e.target.className != "fas fa-list" &&
				e.target.className != "fas fa-list selecionado" && e.target.className != "contentsCategorias") {
...
function abrirlink(from) {
	/* fb://facewebmodal/f?href=https://www.facebook.com/106313655191627/ || fb://page/106313655191627/*/
	if (from == "whatsapp") {
		window.open("https://wa.me/5569992841780");
	} else if (from == "instagram") {
		if (navigator.userAgent.match("Android") || navigator.userAgent.match("iPhone") || navigator.userAgent.match("iPad") || navigator.userAgentData.mobile) {
			//document.querySelector('iframe.teste').src="instagram://user?username=miss.store.pvh";
			let checkapp = window.open("instagram://user?username=miss.store.pvh");
			setTimeout(function() {
				/* window.document.hidden || window.document.webkitHidden ? window.open("https://instagram.com/miss.store.pvh") : null; */
				 checkapp.location.hostname ? checkapp.location.hostname == '' ? checkapp.location.href = "https://instagram.com/miss.store.pvh" : null : checkapp.location.href = "https://instagram.com/miss.store.pvh";
			}, 1000);
		} else {
		...
		if (navigator.userAgent.match("Android") || navigator.userAgent.match("iPhone") || navigator.userAgent.match("iPad") || navigator.userAgentData.mobile) {
			//document.querySelector('iframe.teste').src="fb://facewebmodal/f?href=https://www.facebook.com/106313655191627/";
			let checkapp = window.open("fb://facewebmodal/f?href=https://www.facebook.com/106313655191627/");
			setTimeout(function() {
				/* window.document.hidden || window.document.webkitHidden ? window.open("https://www.facebook.com/106313655191627") : null; */
				checkapp.location.hostname ? checkapp.location.hostname == '' ? checkapp.location.href = "https://www.facebook.com/106313655191627" : null : checkapp.location.href = "https://www.facebook.com/106313655191627";
			}, 1000);
		} else {
...
function obterdadosdostorage() {
	for (let i = 0; i < localStorage.length; i++) {
		...
		i == localStorage.length - 1 ? wwm(".valortotal").then(function() {
			fazersoma();
		}) : null;
	}
}
...
wwm(".asimagensfull").then(function() {
	document.querySelector(".asimagensfull").addEventListener("scroll", function() {
		LocalizacoesAtuais = [];
		document.querySelectorAll(".asimagensfull img").forEach(function(e) {
			LocalizacoesAtuais.push(parseInt(e.getBoundingClientRect().x));
		});
		clearTimeout(timerStamp);
		timerStamp = setTimeout(function() {
			LocalizacoesAtuais.forEach(function(item, idx) {
				item >= 0 && item <= 210 || item <= 0 && item >= -150 ? document.querySelectorAll(".asimagensfull img")[idx].scrollIntoView() : null;
			});
		}, 1000);
	});
	document.querySelector(".asimagensfull").addEventListener("touchend", function() {
		LocalizacoesAtuais.forEach(function(item, idx) {
			item >= 0 && item <= 210 || item <= 0 && item >= -150 ? document.querySelectorAll(".asimagensfull img")[idx].scrollIntoView() : null;
		});
	});
...
wwm(".fotofullviewing .fa-window-close").then(function() {
	document.querySelector(".fotofullviewing .fa-window-close").addEventListener("click", function() {
		...
		document.querySelectorAll(".fotofullviewing img").forEach(function(e, i) {
			if (i == 0) {
...
wwm(".fa-list").then(function() {
	document.querySelector(".fa-list").addEventListener("click", function() {
		if (document.querySelector(".categorias.aberto")) {
			...
			selec.classList.contains("fa-home") ? selec.classList.remove("selecionado") : null;
