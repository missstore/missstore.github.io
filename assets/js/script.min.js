	...
	function criarImages(caminho) {
		...
		req.onload = function() {
			JSON.parse(req.response).info.forEach(function(e) {
				...
				function todosflex(){
					let totaldeitens = document.querySelectorAll(".produtodiv").length;
					if(document.querySelectorAll(".produtodiv[style*='display: flex']").length!=totaldeitens){
				...
				img.onload = function() {
					...
					imagestoload.length <= 1 ?
						(
							document.querySelector(".loadercontainer").classList.remove("carregando"), todosflex(), obterdadosdostorage()
						) :
