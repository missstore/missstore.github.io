...
function RequestImagens(path, mutiplos) {
					...
					if (verificarsejatem(this.parentElement.querySelector(".produtonome").textContent)) {
						...
						quantidadeComprando.addEventListener("change", function() {
							...
							if (this.value == 0 || this.value == '') {
								let con = confirm("Deseja apagar este item?\n" + nomedoproduto);
								con == true ? (
										this.parentElement.parentElement.remove(),
										removerdostorage(n),
										fazersoma()
									) :
									(this.value = 1, fazersoma(), adicionardadosnostorage(n, s, n, v, q));
							} else {
								fazersoma();
						...
						IconeTrash.addEventListener("click", function() {
							...
							let con = confirm("Tem certeza que deseja apagar este item?\n" + nomedoproduto);
							con == true ?
								(this.parentElement.parentElement.remove(),
	...
	if (mutiplos == true) {
		...
		document.querySelector(".nome").textContent = path.replaceAll("/", "").toUpperCase();
		criarImages(path);
	}
}
...
wwm(".far.fa-images").then(function() {
	document.querySelectorAll(".far.fa-images").forEach(function(e){
		e.addEventListener("click",function(){
			let nome = this.children[0].textContent;
