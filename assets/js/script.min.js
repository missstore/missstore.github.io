...
function fazersoma() {
	if (document.querySelectorAll(".adicionados .valordoproduto").length > 0) {
		let soma = [], contagem = document.querySelector(".contagem span");
		...
		document.querySelectorAll(".adicionados .valordoproduto").forEach(function(i) {
			...
			contagem.textContent = Number(contagem.textContent) + Number(i.parentElement.parentElement.querySelector("input").value);
		});
		contagem.classList.add("itens");
		...
		document.querySelector(".valortotal").textContent = '';
		document.querySelector(".contagem span").classList.remove("itens");
	}
}
